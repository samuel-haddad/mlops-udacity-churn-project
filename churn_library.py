...
def perform_eda(df):
        ...
        plot_list = [
                (df['Churn'], './images/eda/churn_distribuition.png', "hist"),
                (df['Customer_Age'], './images/eda/costumer_age_distribuition.png', "hist"),
                (df.Marital_Status.value_counts('normalize'), './images/eda//marital_status_distribuition.png', "bar"),
                (df['Total_Trans_Ct'], './images/eda/total_trans_distribuition.png', "histplot"),
                (df[constants.quant_columns].corr(), './images/eda/heatmap.png', "heatmap")
        ]
...
def train_models(X_train, X_test, y_train, y_test):
        ...
        cv_rfc = GridSearchCV(estimator=rfc, param_grid=constants.param_grid, cv=5, error_score='raise')
...
def classification_report_image(y_train,
        ...
        for train, test, pth in zip(train_list, test_list, path_list):
                ...
                plt.text(0.01, 1.25, str(train[0]), {'fontsize': 10}, fontproperties = 'monospace')
                plt.text(0.01, 0.05, str(classification_report(y_train, train[1])), {'fontsize': 10}, fontproperties = 'monospace') # approach improved by OP -> monospace!
                plt.text(0.01, 0.6, str(test[0]), {'fontsize': 10}, fontproperties = 'monospace')
                plt.text(0.01, 0.7, str(classification_report(y_test, test[1])), {'fontsize': 10}, fontproperties = 'monospace') # approach improved by OP -> monospace!
                plt.axis('off')
        ...
        for graph in plot_list:
                ...
                roc_auc = auc(fpr, tpr)
                plt.plot(fpr, tpr, color=graph[2], label = f'{graph[0]} (AUC = %0.2f)' % roc_auc, alpha=0.8)
...
if __name__ == "__main__":
        ...
        feature_importance_plot(rfc_model, X_train, "./images/results/feature_importances.png")